...
export default function ShapeWaves({
  ...
  useEffect(() => {
    ...
    const measureCssSize = () => {
      ...
      return {
        width: rect.width || root.clientWidth || canvas.clientWidth,
        height: rect.height || root.clientHeight || canvas.clientHeight
      };
    };

    const measureSurface = (): [number, number] => {
      dpr = Math.min(window.devicePixelRatio || 1, MAX_DPR);
      const { width, height } = measureCssSize();
      return [Math.max(1, Math.round(width * dpr)), Math.max(1, Math.round(height * dpr))];
    };

    const startFallback2D = () => {
      ...
      const noise = (x: number, y: number, z: number) => {
        ...
        const n00 = hash21(ix + z * 13, iy);
        const n10 = hash21(ix + 1 + z * 13, iy);
        const n01 = hash21(ix + z * 13, iy + 1);
        const n11 = hash21(ix + 1 + z * 13, iy + 1);
      ...
      const draw = (now: number) => {
        ...
        last = now;
        if (!settings.paused) fallbackTime += delta * settings.speed;

        const bg = parseColor(settings.backgroundColor, '#000000');
        const fg = parseColor(settings.color, '#929292');
        const cell = Math.max(6, settings.cellSize);
        const cols = Math.max(1, Math.ceil(width / cell));
        const rows = Math.max(1, Math.ceil(height / cell));
        const size = cell * Math.min(1, Math.max(0.1, settings.dotSize));

        ctx.fillStyle = `rgb(${bg[0] * 255}, ${bg[1] * 255}, ${bg[2] * 255})`;
        ctx.fillRect(0, 0, width, height);
        ctx.fillStyle = `rgb(${fg[0] * 255}, ${fg[1] * 255}, ${fg[2] * 255})`;
        ...
        for (let row = 0; row < rows; row++) {
          for (let col = 0; col < cols; col++) {
            ...
            const radius = Math.pow(Math.pow(qx, 2.5) + Math.pow(qy, 2.5), 1 / 2.5) / Math.pow(2, 1 / 2.5);
            const fade =
              settings.fade > 0
                ? 1 -
                  Math.min(
                    1,
                    Math.max(0, (radius - (1 - settings.fade * 2.2)) / Math.max(0.001, settings.fade * 2.2))
                  )
                : 1;
            if (fade <= 0.04) continue;

            const tone = noise(col * 0.18 / settings.scale, row * 0.18 / settings.scale, fallbackTime * 0.35);
            const band = Math.min(2, Math.floor(((tone - 0.5) * settings.contrast + settings.brightness) * 3));
            const glyph = size * (0.45 + band * 0.275) * fade;
            ctx.globalAlpha = 0.45 + band * 0.22;

            if (settings.shapes === 'circles' || (settings.shapes === 'mixed' && band === 1)) {
              ctx.beginPath();
              ctx.arc(cx, cy, glyph * 0.5, 0, Math.PI * 2);
              ctx.fill();
            } else if (settings.shapes === 'triangles' || (settings.shapes === 'mixed' && band === 0)) {
              ctx.beginPath();
              ctx.moveTo(cx, cy - glyph * 0.55);
              ctx.lineTo(cx + glyph * 0.5, cy + glyph * 0.45);
              ctx.lineTo(cx - glyph * 0.5, cy + glyph * 0.45);
              ctx.closePath();
              ctx.fill();
            } else {
              ctx.fillRect(cx - glyph * 0.45, cy - glyph * 0.45, glyph * 0.9, glyph * 0.9);
            }
          }
        }
        ctx.globalAlpha = 1;
  ...
  return (
    <div
      ref={rootRef}
      className={`shape-waves ${className}`.trim()}
      data-ready={ready}
    ...
    >
      <canvas
        ref={canvasRef}
        className={`shape-waves-canvas${ready ? ' is-ready' : ''}`}
      />
    </div>
  );
}
